...
class DRNDecomposition:
    """
    Fractal D/R/N (Deleted/Retained/New) decomposition of HLLSet evolution.
    ...
    BACKWARD (History Tail):
        D(t) represents tokens that WERE in hll(t) but left.
        To trace their origin, decompose backwards:
        
            D(t) ← from hll(t) = D(t-1) ∪ R(t-1) ∪ N(t-1)
                 ← from hll(t-1) = D(t-2) ∪ R(t-2) ∪ N(t-2)
                 ← ... until convergence
        
        Convergence depth backward = HISTORICAL MEMORY DEPTH
        (How far back do influences persist?)
    
    FORWARD (Prediction Horizon):
        N(t) represents tokens that ARE NEW at time t.
        To project their fate, decompose forwards:
        
            N(t) → contributes to hll(t+1) = D(t+1) ∪ R(t+1) ∪ N(t+1)
                 → contributes to hll(t+2) = ...
                 → ... until convergence
        
        Convergence depth forward = PREDICTION HORIZON
        (How far forward can we reliably project?)
    
    The SAME fractal structure bounds both memory and foresight!
    This symmetry is fundamental to the system's temporal dynamics.
    
    Fractal Convergence:
        The recursive decomposition CONVERGES because:
        
        1. At each level, we decompose sets into smaller subsets
        2. |D_d| + |D_r| + |D_n| ≤ |D| (can't exceed parent)
        3. Eventually |D| + |R| + |N| < ε (convergence threshold)
        4. The recursion terminates at the "noise floor"
        
        This is analogous to wavelet decomposition - each level
        captures finer dynamics until signal exhaustion.
    ...
    def from_evolution(
        ...
        p_bits: int = 10,
        depth: int = 0,
        threshold: float = DRN_CONVERGENCE_THRESHOLD
    ) -> 'DRNDecomposition':
        """
        Compute D/R/N decomposition from two consecutive HLLSets.
        
        Args:
            hll_prev: HLLSet at time t
            hll_curr: HLLSet at time t+1
            p_bits: Precision bits (for empty HLLSet creation)
        ...
        # D = prev \\ curr = prev - R (deleted tokens)
        deleted = hll_prev.diff(retained)
        
        # N = curr \\ prev = curr - R (new tokens)
        new = hll_curr.diff(retained)
        
        # Check convergence
        total = deleted.cardinality() + retained.cardinality() + new.cardinality()
        converged = total < threshold
    ...
    @classmethod
    def empty(cls, p_bits: int = 10) -> 'DRNDecomposition':
        """Create empty D/R/N decomposition (converged by definition)."""
        empty = HLLSet(p_bits=p_bits)
        return cls(deleted=empty, retained=empty, new=empty, depth=0, converged=True)
...
class FractalW:
    ...
    Convergence Property:
        ...
        This is the "noise floor" - no meaningful signal remains.
        
        The actual_depth property tracks where convergence occurred,
        which may be less than max_depth.
    
    Use Case - Perceptron Swarm Entanglement:
        Two perceptrons are "entangled" (substitutable) if their 
        FractalW patterns are similar across time, meaning:
        - Similar Δ(t) series (DFT fingerprints match)
        - Similar D/R/N ratios at each depth
        - Similar convergence_depth (both hit noise floor at same level)
        - If one fails, the other can substitute
    """
    max_depth: int = 3
    p_bits: int = 10
    ...
    # DRN_path is a string like "D", "R", "N", "D_D", "D_R", "D_N", etc.
    ...
    def update_from_decomposition(
        ...
        Args:
            decomp: DRNDecomposition to extract D/R/N from
            depth: Starting depth (usually 0)
            stop_on_convergence: If True, stop when below threshold
        """
        if depth > self.max_depth:
            return
    ...
    def cardinality_vector(self, depth: int = 0) -> List[float]:
        ...
        Returns [|D|, |R|, |N|] at depth 0,
        or [|D_D|, |D_R|, ..., |N_N|] at depth 1, etc.
        """
        paths = self.drn_paths_at_depth(depth)
        return [
            self.layers[depth].get(p, HLLSet(p_bits=self.p_bits)).cardinality()
            for p in paths
        ]
...
class TemporalDRN:
    ...
    │                           │                                     │
    │    ◄── BACKWARD ──────────┼──────────── FORWARD ──►             │
    │    (History Tail)         │         (Prediction Horizon)        │
    │                           │                                     │
    │    D(t) ← D(t-1) ← ...    │   N(t) → R(t+1) → ...               │
    │    (where did D           │   (where will N                     │
    │     come from?)           │    contribute?)                     │
    │                           │                                     │
    │    Convergence =          │   Convergence =                     │
    │    MEMORY DEPTH           │   PREDICTION HORIZON                │
    └─────────────────────────────────────────────────────────────────┘
    
    The symmetry is fundamental:
    - Past and future bounded by same fractal structure
    - Memory depth ≈ prediction horizon (in stable systems)
    - Asymmetry indicates phase transition or instability
    """
    ...
    def update_forward(
        ...
        for i in range(len(hll_future) - 1):
            ...
            if i == 0 and self.current_drn is None:
                self.current_drn = drn
            
            # For forward, we track where N goes
    ...
    def summary(self) -> Dict[str, Any]:
        """Get bidirectional temporal summary."""
        return {
            "memory_depth": self.memory_depth,
            "prediction_horizon": self.prediction_horizon,
            "temporal_symmetry": round(self.temporal_symmetry, 4),
            "current_drn": self.current_drn.summary() if self.current_drn else None,
            "history": self.history.summary(),
            "prediction": self.prediction.summary(),
        }
    
    def __repr__(self) -> str:
        return f"TemporalDRN(memory={self.memory_depth}, horizon={self.prediction_horizon}, sym={self.temporal_symmetry:.2f})"
